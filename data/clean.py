...
def main():
  ...
  covertype_df = pd.read_csv('./raw/covtype.data', delimiter=',', header=None)
  covertype_df.columns = column_names

  # Select rows where Cover_Type label has the Ponderosa Pine (label 3) or Cottonwood/Willow (label 4) value
  reduced_df = covertype_df[(covertype_df['Cover_Type'] == 3) | (covertype_df['Cover_Type'] == 4)].reset_index().drop(columns=['index'])
